...
async function main() {
  ...
  for (const piedra of piedrasData) {
    await prisma.tipoPiedra.upsert({
      ...
      update: { precioCop: piedra.precio },
      create: {
        nombre: piedra.nombre,
        precioCop: piedra.precio,
        esNatural: !piedra.nombre.includes('CZ') && !piedra.nombre.includes('Laboratorio'),
        categoria: piedra.nombre.includes('CZ') || piedra.nombre.includes('Laboratorio')
          ? 'sintética'
          : piedra.precio >= 50000 ? 'preciosa' : 'semipreciosa'
      }
    })
  }
  ...
  const etapas = [
    ...
    { numero: 3, nombre: 'Fundición', descripcion: 'Servicio externo', costoDefault: 66700 },
    { numero: 4, nombre: 'Preparación Esmaltado', descripcion: 'Tiempo y materiales de preparación' },
    { numero: 5, nombre: 'Esmaltado', descripcion: 'Aplicación de esmaltes (3-4g por pieza)', costoDefault: 50000 },
